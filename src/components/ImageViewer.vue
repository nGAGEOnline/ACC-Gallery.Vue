<template>
	<div v-if="data" class="h-100 image-container">
		<h1>{{ data.name }}</h1>
	...
	methods: {
		async loadImages() {
			try {
				...
				this.index = index;
				this.data = this.images[index];
			} catch (error) {
				console.error("Error fetching JSON data:", error);
			}
		},
		loadNextImage() {
			let targetIndex = (this.index + 1) % this.images.length;
			this.$router.push({ name: "ImageViewer", params: { id: targetIndex } });
      this.loadImages();
			// this.imageData = this.images[this.imageIndex];
		},
		loadPreviousImage() {
			let targetIndex = (this.index - 1 + this.images.length) % this.images.length;
			this.$router.push({ name: "ImageViewer", params: { id: targetIndex }});
      this.loadImages();
			// this.imageData = this.images[this.imageIndex];
		},
